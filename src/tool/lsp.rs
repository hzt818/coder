...
fn format_location(uri: &str, range: &serde_json::Value) -> String {
    let start = range
        .get("start")
        .or_else(|| range.as_object().map(|_| range));
        ...
        match op {
            "go_to_definition" | "find_references" | "hover" => {
                ...
                let (lang, _, _) = match language_for_extension(file) {
                    Some(l) => l,
                    None => return ToolResult::err(format!("Unsupported file type: {}", file)),
                };
                let uri = format!(
                    "file:///{}",
                    file.replace('\\', "/").trim_start_matches("file:///")
                );
                ...
                if let Err(e) = handler
                    .start_server(lang, cmd, args_list.iter().map(|s| s.to_string()).collect())
                    .await
                {
                            ...
                            "go_to_definition" => {
                                ...
                                if locations.is_empty() {
                                    ...
                                    for (i, loc) in locations.iter().enumerate() {
                                        let u =
                                            loc.get("uri").and_then(|u| u.as_str()).unwrap_or("");
                    ...
                    Some(client) => {
                            ...
                            .await
                        {
                            ...
                            Err(e) => ToolResult::ok(format!(
                                "LSP documentSymbols failed: {}\nTry: grep -rn 'fn \\|pub ' {}",
                                e, file
                            )),
                        }
                    }
                    None => ToolResult::ok(format!(
                        "No LSP server for '{}'. Try: grep -rn 'fn \\|pub ' {}",
                        lang, file
                    )),
                }
            }
            "workspace_symbols" => {
                ...
                if langs.is_empty() {
                    return ToolResult::ok(format!(
                        "No running LSP servers. Try: grep -rn '{}' src/",
                        query
                    ));
                }
                    ...
                    Some(client) => {
                            ...
                            .await
                        {
                            Ok(items) => {
                                ...
                                if items.is_empty() {
                                    out.push_str("  (no call hierarchy)\n");
                                }
                                ToolResult::ok(out)
                            }
                            Err(e) => ToolResult::ok(format!(
                                "LSP call_hierarchy failed: {}\nTry: grep -rn",
                                e
                            )),
                        }
                    }
...
mod tests {
    ...
    async fn test_empty_op() {
        assert!(!LspTool.execute(serde_json::json!({})).await.success);
    }
    #[tokio::test]
