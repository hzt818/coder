...
impl Tool for RunTestsTool {
    ...
    async fn execute(&self, args: serde_json::Value) -> ToolResult {
        let all_features = args.get("all_features").and_then(|a| a.as_bool()).unwrap_or(false);
        ...
        if !extra_args.is_empty() {
            for token in extra_args.split_whitespace() { cmd.arg(token); }
        }
        ...
        let summary_line = stdout.lines().rev().find(|l| {
            l.contains("test result:") || l.contains("test ") && l.contains("passed")
        });
        ...
        if stdout.len() > max_chars {
            ...
            result.push_str(&format!("\n... (truncated, {} more bytes)", stdout.len() - max_chars));
        } else {
            result.push_str(&stdout);
        }

        if !stderr.is_empty() {
            result.push_str(&format!("\nstderr ({} bytes):\n{}", stderr.len(), &stderr[..stderr.len().min(5000)]));
        }
...
mod tests {
    ...
    #[test]
    fn test_tool_name() { assert_eq!(RunTestsTool.name(), "run_tests"); }

    #[test]
    fn test_tool_schema() { assert!(RunTestsTool.schema().get("properties").is_some()); }
}
