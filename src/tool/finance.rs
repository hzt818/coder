...
impl Tool for FinanceTool {
    ...
    async fn execute(&self, args: serde_json::Value) -> ToolResult {
        let symbol = args
            ...
            .to_uppercase();
        if symbol.is_empty() {
            return ToolResult::err("Symbol is required");
        }
        ...
        match client.get(&quote_url).send().await {
            Ok(resp) if resp.status().is_success() => {
                if let Ok(data) = resp.json::<serde_json::Value>().await {
                    if let Some(quote) = data["quoteResponse"]["result"]
                        .as_array()
                        .and_then(|r| r.first())
                    {
        ...
        match client.get(&chart_url).send().await {
            Ok(resp) if resp.status().is_success() => {
                if let Ok(data) = resp.json::<serde_json::Value>().await {
                    if let Some(meta) = data["chart"]["result"]
                        .as_array()
                        .and_then(|r| r.first())
                        .and_then(|r| r.get("meta"))
                    {
                        ...
                        let name = data["chart"]["result"][0]["meta"]["symbol"]
                            .as_str()
                            .unwrap_or(&resolved);
        ...
        ToolResult::err(format!(
            "Could not fetch quote for '{}'. Verify the symbol.",
            symbol
        ))
    }
...
mod tests {
    ...
    #[test]
    fn test_name() {
        assert_eq!(FinanceTool.name(), "finance");
    }
    #[tokio::test]
    async fn test_empty_symbol() {
        assert!(!FinanceTool.execute(serde_json::json!({})).await.success);
    }
    #[tokio::test]
